 ...
 *   python -m pufferlib.pufferl train puffer_onlyfins
 *   python -m pufferlib.pufferl eval puffer_onlyfins --load-model-path latest
 *   python -m pufferlib.pufferl export puffer_onlyfins --load-model-path latest
 *   cp puffer_onlyfins_weights.bin resources/onlyfins/<your_username>.bin
 *   bash scripts/build_ocean onlyfins local # Test your model in C
 *
 *  PR your .bin to PufferAI/onlyfins. Be sure to add a line to the index.html
...
void update_goals(OnlyFins* env) {
...
void spawn_goal(OnlyFins* env, int x, int y) {
    for (int g=0; g<env->num_goals; g++) {
        Goal* goal = &env->goals[g];
        if (goal->active) {
            continue;
...
void compute_observations(OnlyFins* env) {
    int obs_idx = 0;
    for (int a=0; a<env->num_agents; a++) {
        Agent* agent = &env->agents[a];
        float x = agent->x;
...
void c_render(OnlyFins* env) {
    if (env->client == NULL) {
        InitWindow(env->width, env->height, "PufferLib OnlyFins");
        SetTargetFPS(60);
        env->client = (Client*)calloc(1, sizeof(Client));
        env->client->puffer = LoadTexture("resources/shared/puffers_128.png");
        env->client->star = LoadTexture("resources/onlyfins/star.png");
    }
